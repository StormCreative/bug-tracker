...
abstract class Activerecord
{
    ...
    /**
     * Holds errors occurred when saving
     * 
     * @var array
     */
    public $errors = array();
    ...
    /**
     * Set whether to refresh models data
     * 
     * @var bool
     * @access protected
     */ 
    ...
    /**
     * All columns for the table and associations
     * 
     * @var bool
     * @access protected
     */
    ...
    /**
     * Holds associative data for saving
     * 
     * @var array
     * @access private
     */
    ...
    /**
     * Sets whether a model has image associated
     * 
     * @var bool
     * @access protected
     */
    ...
    /**
     * Sets whether a model has a download associated
     * 
     * @var bool
     * @access protected
     */
    ...
    /**
     * Holds all models has many association tables
     * 
     * @var array
     */
    ...
    /**
     * Holds all models has one association tables
     * 
     * @var array
     */
    ...
    public function all( $binds = array(), $joins = TRUE )
    {
        ...
        if ( count( $output ) > 0 ) {
            ...
            $data = $this->set_has_many_relationship_all($output);
    ...
    /**
     * Gets all has_many relationships and sets them as a class attribute
     * The JOIN/UNION would not work, as we want to get back data as a key in an array
     * for each item that is within the loop.
     * 
     * @param array $data - data returned from a get/find
     * @return array
     */
    private function set_has_many_relationship_all($data)
    {
        if( property_exists(get_class($this), 'has_many') ) {

            $assocs = $this->has_many;
            $main_table = strtolower($this->clean_table());
            ...
            foreach($data as $item) {
                ...
                foreach($this->has_many as $many) {
                    ...
                    $query = 'SELECT *'.$add_query.' FROM '.$many_table.' WHERE '.$main_table.'_id = :id';

                    // Unset the add query for the next loop
                    $add_query = "";

                    $has_many = $this->table()->query->getAssoc($query, array('id' => $id));

                    // If theres a joint query we set it in the array
                    if ( !!$has_many ) {
                        $data[$count][$many] = $has_many;
                    }
